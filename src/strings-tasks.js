...
function isPalindrome(str) {
  const strClean = str.replace(/[^A-Za-z]/g, '').toLowerCase();
...
function findLongestWord(str) {
  const atom = str.split(' ');
  let res = '';
  for (let i = 0; i < atom.length; i += 1) {
    if (atom[i].length > res.length) {
      res = atom[i];
    }
  }
  return res;
...
function invertCase(str) {
  const atom = str.split('');
  for (let i = 0; i < atom.length; i += 1) {
    if (atom[i] === atom[i].toLowerCase()) {
      atom[i] = atom[i].toUpperCase();
    } else {
      atom[i] = atom[i].toLowerCase();
    }
  }
  return atom.join('');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (l) => rot13[input.indexOf(l)]);
}
